...
float PathingNode::GetCostFromNode(PathingNode* pFromNode)
{
	...
	assert(pArc);
	PathfindingMath::Vec3 diff = pFromNode->GetPos() - m_pos;
...
// PathPlan
//--------------------------------------------------------------------------------------------------------
bool PathPlan::CheckForNextNode(const PathfindingMath::Vec3& pos)
{
	if (m_index == m_path.end())
		return false;

	PathfindingMath::Vec3 diff = pos - (*m_index)->GetPos();
...
void PathPlanNode::UpdateHeuristics(void)
{
	...
	// heuristic (h)
	PathfindingMath::Vec3 diff = m_pPathingNode->GetPos() - m_pGoalNode->GetPos();
...
//
std::shared_ptr<PathPlan> AStar::operator()(PathingNode* pStartNode, PathingNode* pGoalNode)
{
	assert(pStartNode);
	assert(pGoalNode);
...
PathPlanNode* AStar::AddToOpenSet(PathingNode* pNode, PathPlanNode* pPrevNode)
{
	...
	std::shared_ptr<PathPlanNode> pThisNode = NULL;
	if (it == m_nodes.end())
	{
		pThisNode = std::make_shared<PathPlanNode>(pNode, pPrevNode, m_pGoalNode);
...
std::shared_ptr<PathPlan> AStar::RebuildPath(PathPlanNode* pGoalNode)
{
	assert(pGoalNode);

	std::shared_ptr<PathPlan> pPlan = std::make_shared<PathPlan>();
...
PathingNode* PathingGraph::FindClosestNode(const PathfindingMath::Vec3& pos)
{
	...
	for (PathingNodeVec::iterator it = m_nodes.begin(); it != m_nodes.end(); ++it)
	{
		...
		PathfindingMath::Vec3 diff = pos - pNode->GetPos();
...
PathingNode* PathingGraph::FindFurthestNode(const PathfindingMath::Vec3& pos)
{
	// This is a simple brute-force O(n) algorithm that could be made a LOT faster by utilizing
	// spatial partitioning, like an octree (or quadtree for flat worlds) or something similar.
...
std::shared_ptr<PathPlan> PathingGraph::FindPath(const PathfindingMath::Vec3& startPoint, const PathfindingMath::Vec3& endPoint)
{
...
std::shared_ptr<PathPlan> PathingGraph::FindPath(const PathfindingMath::Vec3& startPoint, PathingNode* pGoalNode)
{
	PathingNode* pStart = FindClosestNode(startPoint);
	return FindPath(pStart, pGoalNode);
}

std::shared_ptr<PathPlan> PathingGraph::FindPath(PathingNode* pStartNode, const PathfindingMath::Vec3& endPoint)
{
	PathingNode* pGoal = FindClosestNode(endPoint);
	return FindPath(pStartNode, pGoal);
}

std::shared_ptr<PathPlan> PathingGraph::FindPath(PathingNode* pStartNode, PathingNode* pGoalNode)
{
...
void PathingGraph::BuildTestGraph(void)
{
	...
	for (float x = -45.0f; x < 45.0f; x += 10.0f)
	{
		...
		{
			// add the new node
			PathingNode* pNode = new PathingNode(PathfindingMath::Vec3(x, 0, z));
...
void PathingGraph::LinkNodes(PathingNode* pNodeA, PathingNode* pNodeB)
{
	...
	auto pArc = std::make_shared<PathingArc>();
...
// Global functions
//--------------------------------------------------------------------------------------------------------
std::shared_ptr<PathingGraph> CreatePathingGraph(void)
{
	auto pPathingGraph = std::make_shared<PathingGraph>();
