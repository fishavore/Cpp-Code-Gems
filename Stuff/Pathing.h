...
{
	float m_tolerance;
	PathfindingMath::Vec3 m_pos;
	PathingArcList m_arcs;

public:
	explicit PathingNode(const PathfindingMath::Vec3& pos, float tolerance = PATHING_DEFAULT_NODE_TOLERANCE) : m_pos(pos) { m_tolerance = tolerance; }
	const PathfindingMath::Vec3& GetPos(void) const { return m_pos; }
...
public:
	...
	const PathfindingMath::Vec3& GetCurrentNodePosition(void) const { assert(m_index != m_path.end()); return (*m_index)->GetPos(); }
	bool CheckForNextNode(const PathfindingMath::Vec3& pos);
...
public:
	...
	std::shared_ptr<PathPlan> operator()(PathingNode* pStartNode, PathingNode* pGoalNode);
...
private:
	...
	std::shared_ptr<PathPlan> RebuildPath(PathPlanNode* pGoalNode);
};
...
public:
	...
	PathingNode* FindClosestNode(const PathfindingMath::Vec3& pos);
	PathingNode* FindFurthestNode(const PathfindingMath::Vec3& pos);
	PathingNode* FindRandomNode(void);
	std::shared_ptr<PathPlan> FindPath(const PathfindingMath::Vec3& startPoint, const PathfindingMath::Vec3& endPoint);
	std::shared_ptr<PathPlan> FindPath(const PathfindingMath::Vec3& startPoint, PathingNode* pGoalNode);
	std::shared_ptr<PathPlan> FindPath(PathingNode* pStartNode, const PathfindingMath::Vec3& endPoint);
	std::shared_ptr<PathPlan> FindPath(PathingNode* pStartNode, PathingNode* pGoalNode);
